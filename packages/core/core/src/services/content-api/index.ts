 ...
 * singletons loaded once via native `require`. Under the experimental in-process
 * Vite reload the SAME process re-runs route registration on every reload, so
 * `applyExtraParamsToRoutes` runs a SECOND time over a route that already carries
 * the first boot's merged params. Without this marker the duplicate-param guard
 * would throw `param "…" already exists on route` on reload.
 *
