...
describe('Content API - Permissions', () => {
  ...
  describe('addQueryParams / addInputParams / applyExtraParamsToRoutes', () => {
    ...
    // Regression: the experimental in-process Vite reload re-runs route
    // registration over the SAME persistent route singleton. applyExtraParams
    // must be idempotent on re-application — it must NOT re-throw the
    // duplicate-param guard for params IT already merged.
