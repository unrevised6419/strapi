...
describe('registerRoutes — default-path route metadata (regression)', () => {
  ...
  it('re-running registration over the SAME source singletons is idempotent (reload-safe)', () => {
    ...
    registerRoutes(strapi);
    // Simulate the experimental in-process reload re-registering the persistent
    // (native-require'd) route singletons: must not throw `param already exists`
    // and must leave the source deterministically tagged.
